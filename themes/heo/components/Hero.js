...
function Banner(props) {
  ...
  return (
    <div
      ...
      className='hidden xl:flex xl:flex-col group h-full bg-[var(--heo-card)] rounded-xl border border-[var(--heo-card-border)] dark:border-white/10 heo-card mb-3 relative overflow-hidden'>
      <div
        id='banner-title'
        className='z-10 flex flex-col absolute top-10 left-10'>
        <div className='text-4xl font-bold mb-3 text-[var(--heo-text)] dark:text-white'>
      ...
      <div
        ...
        className={
          'z-20 rounded-xl overflow-hidden opacity-0 group-hover:opacity-100 duration-300 transition-all bg-[#425aefdd] dark:bg-[#dca846] dark:text-white cursor-pointer absolute w-full h-full top-0 flex justify-start items-center'
        }>
          ...
          return (
            <div key={index} className='tags-group-icon-pair ml-6 select-none'>
              <div
                ...
                className={
                  'tags-group-icon w-28 h-28 rounded-3xl flex items-center justify-center text-white text-lg font-bold shadow-[0_8px_12px_-3px_rgba(0,0,0,0.15)]'
                }>
                <LazyImage
                  priority={true}
                  src={g.img_1}
...
function GroupMenu() {
  ...
  return (
    <div className='h-[165px] select-none xl:h-20 flex flex-col justify-between xl:space-y-0 xl:flex-row w-28 lg:w-48 xl:w-full xl:flex-nowrap xl:space-x-3'>
      <SmartLink
        href={url_1}
        className='group relative overflow-hidden bg-gradient-to-r from-[#425AEF] to-[#7B8DF3] flex h-20 justify-start items-center text-white rounded-xl xl:hover:w-1/2 xl:w-1/3 transition-all duration-500 ease-in'>
      ...
      <SmartLink
        href={url_2}
        className='group relative overflow-hidden bg-gradient-to-r from-[#FB7185] to-[#FB9A6C] flex h-20 justify-start items-center text-white rounded-xl xl:hover:w-1/2 xl:w-1/3 transition-all duration-500 ease-in'>
        <div className='font-bold lg:text-lg pl-5 relative -mt-2'>
          {title_2}
          <span className='absolute -bottom-0.5 left-5 w-5 h-0.5 bg-white rounded-full'></span>
        </div>
      ...
      <SmartLink
        href={url_3}
        className='group relative overflow-hidden bg-gradient-to-r from-[#38BDF8] to-[#22D3EE] hidden h-20 xl:flex justify-start items-center text-white rounded-xl xl:hover:w-1/2 xl:w-1/3 transition-all duration-500 ease-in'>
        ...
        {topPosts?.map((p, index) => {
          return (
            <SmartLink href={`${siteConfig('SUB_PATH', '')}/${p?.slug}`} key={index}>
              <div className='cursor-pointer h-[164px] group relative flex flex-col w-52 xl:w-full overflow-hidden bg-[var(--heo-card)] dark:text-white rounded-xl heo-card heo-card-hover border border-[var(--heo-card-border)] dark:border-white/10'>
                <LazyImage
                  priority={index === 0}
                ...
                {/* hover 悬浮的 ‘荐’ 字 */}
                <div className='opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 duration-200 transition-all absolute -top-2 -left-2 bg-[var(--heo-main)] dark:bg-yellow-600 text-white rounded-xl overflow-hidden pr-2 pb-2 pl-4 pt-4 text-xs'>
